...
def blackbody_spectral_radiance(energy, T):
    ...
    planck_spectrum = bb_constant * energy**3 * 1.0 / (planck_spectrum_exp - 1)
...
class SED:
    ...
    ENERGY_MAX = 200.0  # keV
    ENERGY_MAX_ERG = convert_units(ENERGY_MAX * u.keV, u.erg)
    ENERGY_RANGE_NUM_BINS = 1000
    ENERGY_BIN_EDGES_KEV = np.geomspace(
        ENERGY_MIN, ENERGY_MAX, ENERGY_RANGE_NUM_BINS + 1
    )
    ENERGY_BIN_WIDTHS_KEV = np.diff(ENERGY_BIN_EDGES_KEV)
    ENERGY_RANGE_KEV = (
        ENERGY_BIN_EDGES_KEV[:-1]
        + (ENERGY_BIN_EDGES_KEV[1:] - ENERGY_BIN_EDGES_KEV[:-1]) / 2.0
    )
    ENERGY_RANGE_ERG = convert_units(
        ENERGY_RANGE_KEV * u.keV, u.erg
    )  # np.geomspace(ENERGY_MIN_ERG, ENERGY_MAX_ERG, ENERGY_RANGE_NUM_BINS)
    ELECTRON_REST_MASS = 511.0  # kev
    ...
    def __init__(
        ...
        # model parameters
        self.hard_xray_fraction = (
            hard_xray_fraction  # fraction of energy in Eddington units in the corona.
        )
        self.corona_electron_energy = (
            corona_electron_energy  # temperature of the corona's electrons in keV.
        )
        self.warm_electron_energy = (
            warm_electron_energy  # temperature of the soft region's electrons in keV.
        )
        self.warm_photon_index = (
            warm_photon_index  # powerlaw index of the warm component.
        )
        self.reflection_albedo = (
            reflection_albedo  # reflection albedo for the reprocessed flux.
        )
        self.corona_radius = self.corona_find_radius
        self.corona_height = min(100.0, self.corona_radius)
    ...
    def disk_nt_temperature4(self, r):
        ...
        nt_constant = (
            ...
            * const.m_p
            * const.c**5
            / (2 * const.sigma_sb * const.sigma_t * const.G * const.Ms)
        )
    ...
    def disk_number_density(self, r):
        ...
        cut = (
            18
            * (self.M) ** (2.0 / 21.0)
            * (self.mdot / self.efficiency) ** (16.0 / 21.0)
            * 2
            * self.Rg
        )
        if R <= cut:
            ...
            rho = (
                4.66
                * (self.M) ** (-7.0 / 10.0)
                * (self.mdot / self.efficiency) ** (2.0 / 5.0)
                * (R / (2 * self.Rg)) ** (-33.0 / 20.0)
            )
    ...
    def disk_gas_pressure(self, r):
        ...
        temperature = self.disk_nt_temperature4(r) ** (1.0 / 4.0)
    ...
    def reprocessed_flux(self, radius):
        ...
        aux = 3.0 * const.G * M / (8 * np.pi * R**3.0)
        aux *= 2 * Lhot / (const.c**2)
        aux *= H / (6 * self.Rg) * (1.0 - a)
        aux *= (1.0 + (H / R) ** 2) ** (-3.0 / 2.0)
        return aux
    ...
    def disk_spectral_radiance(self, energy, r):
        T = self.disk_temperature4(r) ** (1.0 / 4.0)
    ...
    def disk_spectral_luminosity(self, energy):
        ...
        radial_integral = (
            ...
            * self.Rg**2
            * integrate.quad(
                lambda r: r * self.disk_spectral_radiance(energy, r),
                self.warm_radius,
                self.gravity_radius,
            )[0]
        )  # units of 1 / s
    ...
    def disk_luminosity(self):
        ...
        lumin = (
            constant
            * integrate.quad(
                lambda r: r * self.disk_temperature4(r),
                self.warm_radius,
                self.gravity_radius,
            )[0]
        )
    ...
    def disk_truncated_luminosity(self, r_min, r_max):
        ...
        lumin = (
            constant
            * integrate.quad(lambda r: r * self.disk_nt_temperature4(r), r_min, r_max)[
                0
            ]
        )
        return lumin
    ...
    def disk_flux(self, distance):
        ...
        """
        return (
            self.disk_sed / self.ENERGY_RANGE_KEV / (4 * np.pi * distance**2)
        )  # keV / keV / s / cm^2
    ...
    def disk_flux_r(self, r, dr, distance, exclude_warm_region=True):
        ...
        disk_energy_flux_bin = np.pi * self.disk_spectral_radiance(
            self.ENERGY_RANGE_ERG, r
        )  # flux in mid bin units 1 / cm^2 / s
        disk_energy_flux = (
            disk_energy_flux_bin / self.ENERGY_RANGE_KEV
        )  # 1 / cm^2 / s / keV
        disk_lumin = (
            4 * np.pi * (self.Rg) ** 2 * r * dr * disk_energy_flux
        )  # 1 / s / keV
        disk_energy_flux = (
            disk_lumin / (4 * np.pi * distance**2) * self.ENERGY_RANGE_KEV
        )  # keV / cm^2 / s / keV
    ...
    def corona_find_radius(self):
        ...
        try:
            ...
            print(
                "Accretion rate is too low to power a corona. Radius is smaller than last circular stable orbit."
            )
            corona_radius = 0
    ...
    def corona_seed_luminosity(self):
        ...
        integral = integrate.quad(
            lambda r: r * self.disk_temperature4(r) * self._corona_covering_factor(r),
            self.corona_radius,
            self.gravity_radius,
        )[0]
    ...
    def corona_photon_index(self):
        ...
        gamma_cor = (
            7.0
            / 3.0
            * (self.corona_dissipated_luminosity / self.corona_seed_luminosity)
            ** (-0.1)
        )
    ...
    def compton_photon_flux(self, ear, params):
        photon_number_per_bin = donthcomp(
            ear=self.ENERGY_RANGE_KEV, param=params
        )  # units of Photons / cm^2 / s
        photon_number_flux = (
            photon_number_per_bin / self.ENERGY_RANGE_KEV
    ...
    def corona_photon_flux(self):
        ...
        t_corona = self.disk_temperature4(self.corona_radius) ** (1.0 / 4.0) * const.k_B
        t_corona_kev = convert_units(t_corona * u.erg, u.keV)
        ywarm = (4.0 / 9.0 * self.warm_photon_index) ** (-4.5)
        ...
        photon_number_flux = self.compton_photon_flux(
            ear=self.ENERGY_RANGE_KEV, params=params
        )
        return photon_number_flux
    ...
    def corona_flux(self, distance):
        ...
        flux = np.sum(
            self.ENERGY_BIN_WIDTHS_KEV * photon_number_flux * self.ENERGY_RANGE_KEV
        )  # photons / cm^2 /s
        ...
        flux = (
            ratio * photon_number_flux[mask] * self.ENERGY_RANGE_KEV[mask]
        )  # units of keV / cm^2 / s / keV
    ...
    def warm_flux_r(self, r, dr, distance):
        ...
        t_warm = self.disk_temperature4(r) ** (1.0 / 4.0) * const.k_B
        t_warm_kev = convert_units(t_warm * u.erg, u.keV)
        params = [gamma, kt_e, t_warm_kev, 0, 0]
        photon_flux_r = self.compton_photon_flux(
            ear=self.ENERGY_RANGE_KEV, params=params
        )  # 1 / cm^2 / s / keV
        ...
        energy_flux_r_integrated = np.sum(
            self.ENERGY_BIN_WIDTHS_KEV[mask]
            * photon_flux_r[mask]
            * self.ENERGY_RANGE_KEV[mask]
        )  # keV / cm^2 / s
        energy_flux_r_integrated = convert_units(
            energy_flux_r_integrated * u.keV, u.erg
        )
        # we then normalize the flux using the local disc energy flux.
        disk_lumin = 4 * np.pi * (self.Rg) ** 2.0 * r * dr * self.disk_radiance(r)
        disk_flux = disk_lumin / (4.0 * np.pi * distance**2)
    ...
    def warm_flux(self, distance):
        ...
        """
        r_range = np.linspace(
            self.corona_radius, self.warm_radius, 500
        )  # the soft-compton region extends form Rcor to 2Rcor.
    ...
    def compute_uv_fraction_radial(self, r, dr, distance):
        ...
        int_total_flux = integrate.simps(
            x=self.ENERGY_RANGE_KEV[mask], y=total_flux[mask]
        )
        int_total_flux_uv = integrate.simps(
            x=self.ENERGY_RANGE_KEV[uv_mask], y=total_flux[uv_mask]
        )
    ...
    def compute_uv_fractions(
        self,
        inner_radius="warm_radius",
        outer_radius=1000,
        distance=1e20,
        log_spaced=False,
        include_corona=False,
        return_all=True,
    ):
        """
        ...
        if log_spaced:
            r_range = np.geomspace(
                inner_radius, outer_radius, self.number_bins_fractions
            )
        ...
        dr_range = np.diff(
            r_range, append=border_value
        )  # self.uv_fraction_radius_range[1] - self.uv_fraction_radius_range[0]
        fraction_list = []
        total_uv_flux = 0
        total_flux = 0
        ...
        for i, r in enumerate(r_range):
            ...
            (
                ...
                int_total_flux,
                component_fractions,
            ) = self.compute_uv_fraction_radial(r, dr, distance)
        ...
        if return_all:
            return [
                ...
                total_uv_flux,
                total_flux,
                np.array(component_fractions_list),
            ]
        else:
